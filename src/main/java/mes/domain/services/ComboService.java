...
public class ComboService {
	...
	ComboDataFunction data_year=(String cond1, String cond2, String cond3) -> {
		String sql = """
				...
				union all
				select FORMAT(DATEADD(YEAR, -1, GETDATE()), 'yyyy') as value , FORMAT(DATEADD(YEAR, -1, GETDATE()), 'yyyy') as text
				union all
				select FORMAT(DATEADD(YEAR, -2, GETDATE()), 'yyyy') as value , FORMAT(DATEADD(YEAR, -2, GETDATE()), 'yyyy') as text  
				""";
		MapSqlParameterSource dicParam = new MapSqlParameterSource();
        dicParam.addValue("cond1", cond1);
        dicParam.addValue("cond2", cond2);
